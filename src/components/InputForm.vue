<template>
    ...
    <div class="input-form" style="background-color: #FEFFCD;">
      <h1 v-b-tooltip.hover.topright title="inputForm child component">im the form {{msg}}</h1>
...
import Vuex from "vuex"; // the getter required vuex, so had to bring it in here.
  ...
  created() {
    this.loading = true;
    this.$store.dispatch('form/loadItems')
      .then((response) => {
        this.loading = false;
      })
      .catch((error) => {
  ...
  computed: Vuex.mapGetters({
    ...
    newItemLength: 'form/newItemLength',
    isNewItemInputLimitExceeded: 'form/isNewItemInputLimitExceeded',
    email: 'form/email',
    urgency: 'form/urgency',
    isNotUrgent: 'form/isNotUrgent',
  ...
  methods: {
    onInputChange(evt) {
      ...
      const value =
        element.name === "TERMS_AND_CONDITIONS"
          ? element.checked
          : element.value;
      this.$store.commit(`form/UPDATE_${element.name}`, value);
    },
    submitForm(evt) {
      evt.preventDefault();

      this.fieldErrors = this.validateForm(this.$store.state.form.fields);
      if (Object.keys(this.fieldErrors).length) return;

      const items = [
        ...this.$store.state.form.items,
        this.$store.state.form.fields.newItem
      ];

      this.saveStatus = 'SAVING';

      this.$store.dispatch('form/saveItems', items)
        .then(() => {
    ...
    validateForm(fields) {
      const errors = {};
      if (!this.$store.state.form.fields.newItem) {
        errors.newItem = "New Item Required";
      }
      if (!this.$store.state.form.fields.email) {
        errors.email = "Email Required";
      }
      if (!this.$store.state.form.fields.urgency) {
        errors.urgency = "Urgency Required";
      }
      if (!this.$store.state.form.fields.termsAndConditions) {
        errors.termsAndConditions = "Terms and conditions have to be approved";
      }

      if (this.$store.state.form.fields.email &&
        !this.isEmail(this.$store.state.form.fields.email)) {
